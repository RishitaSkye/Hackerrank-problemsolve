...
int main(){
    int n;  
    scanf("%d",&n);
    int arr[n]; //arr to store non zero elements
    int j=0;  //counter for non zero elements
    for(int i=0;i<n;i++){   //taking i/p and storing non zero elements
        int a;
        scanf("%d",&a);
        if(a!=0){
           arr[j]=a;   //store non-zero elements
           j++;  //increment counter
        }
    }
    for(int i=0;i<n;i++){  //print non zero elements
        if(i<j){
            printf("%d", arr[i]);
        }
        else{
            printf("0");  //print remaining zeros at ladt
        }
    }
